...
`EventApplyResult` 新增 `reducerErrors?: ReducerError[]` 字段（结构化诊断），
让上层（session / UI）能展示完整的 reducer 拒绝原因列表。每个 `ReducerError`
包含 `code`（按错误类型分类）、`message`（保留原始错误文本）与可选的 `entityPath`
（格式 `"entityType:entityId"`），便于 UI 精确定位错误来源。

```ts
type ReducerErrorCode =
  | "entity_not_found"      // entity 不存在
  | "invalid_transition"    // 状态机转换不允许（含 cascade failures）
  | "constraint_violation"  // 业务规则违反
  | "validation_error";     // 未知事件类型或校验失败

interface ReducerError {
  code: ReducerErrorCode;
  message: string;
  entityPath?: string;  // "entityType:entityId"
}
```

`AcceptedEventListener` 同步扩展为 `(event, result) => void`，把 `EventApplyResult`
完整透传给上层（session → UI hook）。`useOfficeState` 据此从 `result.reducerErrors`
读取结构化诊断并展示。`rebuildFromLog` 遵循同一逻辑，保证重放幂等一致。
