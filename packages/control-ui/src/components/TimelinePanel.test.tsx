...
function makeTimeline(events: { eventId: string; type: string; timestamp: string; payload: Record<string, unknown> }[]): TimelineIntegrationView {
  return { events };
}
...
describe("TimelinePanel", () => {
  it("renders timeline events in order", () => {
    const timeline = makeTimeline([
      { eventId: "e2", type: EventType.REVIEW_ASSIGNED, timestamp: "2026-01-01T00:01:00Z", payload: { agentId: "a1" } },
      { eventId: "e1", type: EventType.TASK_CREATED, timestamp: "2026-01-01T00:00:00Z", payload: { taskId: "t1" } },
    ]);
  ...
  it("summarizes task and review events", () => {
    const timeline = makeTimeline([
      { eventId: "e1", type: EventType.TASK_CREATED, timestamp: "2026-01-01T00:00:00Z", payload: { taskId: "t1" } },
      { eventId: "e2", type: EventType.REVIEW_ASSIGNED, timestamp: "2026-01-01T00:01:00Z", payload: { agentId: "a1" } },
      { eventId: "e3", type: EventType.AUDIT_NOTE_ADDED, timestamp: "2026-01-01T00:02:00Z", payload: { author: "auditor-1" } },
    ]);
